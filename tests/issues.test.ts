...
import { fetchCommunityIssues, closeIssueWithComment, isValidRepo, isSafeIssueNumber, detectRepo, syncReactionsToItems, ISSUES_PER_PAGE, FETCH_TIMEOUT_MS, ISSUES_DEFAULT_ACTION, SYNC_REACTIONS_TIMEOUT_MS } from "../src/issues.js";
...
describe("SYNC_REACTIONS_TIMEOUT_MS", () => {
  it("is 15000 (value-pinning)", () => {
    expect(SYNC_REACTIONS_TIMEOUT_MS).toBe(15_000);
  });
});

describe("ISSUES_DEFAULT_ACTION", () => {
  it('is "closed" (value-pinning)', () => {
    expect(ISSUES_DEFAULT_ACTION).toBe("closed");
  });
});
