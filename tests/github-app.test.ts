...
describe("github-app", () => {
  ...
  describe("githubGraphQL", () => {
    // Helper: prime the token cache so githubGraphQL doesn't need
    // a separate fetch for getInstallationToken.
    async function setupWithToken() {
      // First call = getInstallationToken, second = the GraphQL call
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ data: { viewer: { login: "bloom-bot" } } }),
        });
      return loadModule();
    }

    it("sends POST to /graphql with query and variables in body", async () => {
      const { githubGraphQL } = await setupWithToken();
      await githubGraphQL("query { viewer { login } }", { owner: "test" });
    ...
    it("uses bearer token authorization", async () => {
      ...
      const [, opts] = mockFetch.mock.calls[1] as [string, RequestInit];
      expect((opts.headers as Record<string, string>).Authorization).toBe("bearer ghs_graphql");
    });

    it("includes Content-Type and API version headers", async () => {
      const { githubGraphQL } = await setupWithToken();
      await githubGraphQL("query { viewer { login } }");
    ...
    it("returns parsed JSON response", async () => {
      ...
      expect(result).toEqual({ data: { viewer: { login: "bloom-bot" } } });
    });
    ...
    it("throws on non-ok response with status and body", async () => {
      ...
      const { githubGraphQL } = await loadModule();
